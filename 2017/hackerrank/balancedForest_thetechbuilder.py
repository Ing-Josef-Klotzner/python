...
def populate_tree_sums (root):
    ...
    while stack:
        ...
        if selected_node not in visited:
            ...
            selected_node.total_sum = sum (map (
                lambda tn: tn.total_sum, selected_node.children
                )) + selected_node.value
...
def find_best_balanced_forest (root):
    ...
    while stack:
        ...
        if selected_node not in visited:
            ...
            # if the cut is made in selected subtree and the visited subtree and
            # (in case the comp or sum exists in the visited sums)
            # the remaining subtree sum is equal or less than the sums 
            # of the current and the visited subtrees (which are equal)
            # I can balance the remaining only with 0 or positive elements
            if ((selected_node.total_sum * 2) in visited_sums or 
            ...
            if ((selected_node.total_sum in visited_sums or
                selected_node.total_sum in root_complement_sums) and
                selected_node.total_sum * 3 >= root.total_sum):
                # candidate split:
                candidate_value = selected_node.total_sum * 3 - root.total_sum
                if candidate_value < min_result_value:
                    min_result_value = candidate_value
