...
1
(((((((((((((((a|b)*)a)(a|b))(a|b))(a|b))(a|b))(a|b))(a|b))(a|b))(a|b))(a|b))(a|b))(a|b))(a|b)) 1000000000
               =
1
(((a|b)*)a)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b)(a|b) 1000000000
out: 570312504

2^(1000000000-13) * 2 ^12 = 2^999999999
2^999999999 % (1e9 + 7) = 570312504

(a|b)* => 2^N
((a*)(b(a*))) => N
(a|b)  => *2

ε (U+03B5)  The empty string matches epsilon
x           The character x matches the pattern x, for any character x
(r1|r2))    The string st match (r1|r2) if st matches either r1 or r2
(r1r2)      The string st match (r1r2) if st can be split into two
            substrings st1 and st2, st = st1 ++ st2, so that st1 matches
            r1 and st2 matches r2
(r)*        The string st will match (r)* if st can be split into zero
            or more substrings, st = st1 ++ st2 ++ ... ++ stn, each of 
            which matches r. The zero case implies that the empty string
            will match (r)* for any regular expression r.
"""
# To compute x^y under modulo m 
def power (x, y, m = 10 ** 9 + 7):
    if (y == 0): return 1
    p = power (x, y // 2, m) % m
    p = (p * p) % m
    if y % 2 == 0: return p
...
class CountStrings:
    def __init__ (self, regex_string):
        ...
        print ("translating to dfa graph ...")
        self.dfa_graph = translate_graph.translate ()
        print ("dfa graph finished")
    ...
    def translate_regex (self, index = 0):
        ...
        while index < len (self.regex_string):
            ...
            else: result_set.insert (RegexGraphNFA (Alphabet [self.regex_string [index]]))
            index += 1
...
class ResultSet:
    AND, OR, REPEAT = 1, 2, 3
    def __init__ (self):
        self.r1 = None; self.r2 = None
        self.op = self.AND
    def set_or (self): self.op = self.OR
    def set_repeat (self): self.op = self.REPEAT
    def calculate_result (self):
        if self.op == self.REPEAT:
            self.calculate_repeat ()
        elif self.r2 is None: pass
        elif self.op == self.OR: self.calculate_or ()
        else: self.calculate_and ()
        return self.r1
    def calculate_repeat (self): self.r1.graph_repeat ()
    def calculate_or (self): self.r1.graph_or (self.r2)
    def calculate_and (self): self.r1.graph_add (self.r2)
    def insert (self, value):
        if self.r1 is None: self.r1 = value
        else: self.r2 = value

class RegexGraphNFA:
    node_count = 0
    def __init__ (self, in_char):
        self.edges = None; self.head = None; self.tail = None
...
class Matrix:
    ...
    def __str__ (self):
        my_str = ''
        for row in self.matrix: my_str += str(row) + "\n"
        return my_str
    def pad_zeros (self):
        self.matrix = []
        for i in range (self.dimension):
    ...
    def pow (self, pow_val, mod_val = None):
        ...
        while pow_val > 0:
            ...
            else: current_pow_matrix = current_pow_matrix.mat_square_mult (current_pow_matrix, mod_val)
...
def countStrings (r, l):
    ...
    # Klammern auflösen
    stack = []
    kl = []   # [(hNr, vNr, Start, Stop), ..]
#    start = []
    hNr = -1
    vNr = -1
    for i in range (n):
        if r [i] == '(':
            if vNr == -1 or stack and stack [-1] == 0: 
                vNr += 1
            if hNr == -1:
                hNr += 1
            #stack.append ((hNr, vNr, i, 0))
            stack.append (i)
        if r [i] == ')':
            kl.append ((stack.pop (), i))
    print (stack)
    print (kl)
...
def main ():
    ...
    for _ in range (t):
        ...
        result = my_class.calculate (int (l))
