...
// compile:
// g++ kittysCalculationsOnTree.cpp -std=c++11 -o kittysCalculationsOnTree

// based on centroid decomposition
...
void dfs2 (int id) {  // fills size array for centroid weighting
    v.push_back (id);
    visit [id] = 1;
    sz [id] = 1;
    mx [id] = 0;
...
int get_cen (int id) {  // get centroid
    v.clear ();
    dfs2 (id);
    int tot = SZ (v);
    int cen = -1;
...
// puts original distances /levels (0 = orig. root) to centroid tree
