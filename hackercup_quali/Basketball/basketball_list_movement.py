...
#                  team rotate by moving 1 player from/to team_a_pl and team_a_bench lists
                    if len(team_a_bench) > 0:
                        ...
                        team_a_bench.sort(key=itemgetter(2),reverse=False)
                        team_a_bench.sort(key=itemgetter(1),reverse=False)
                        team_a_bench.sort(key=itemgetter(3),reverse=True)
                        team_a_pl.append(team_a_bench.pop())
                        team_a_pl.sort(key=itemgetter(2),reverse=True)
                        team_a_pl.sort(key=itemgetter(1),reverse=True)
...
#                  team rotate by moving 1 player from/to team_b_pl and team_b_bench lists
                    if len(team_b_bench) > 0:
                        ...
                        team_b_bench.sort(key=itemgetter(2),reverse=False)
                        team_b_bench.sort(key=itemgetter(1),reverse=False)
                        team_b_bench.sort(key=itemgetter(3),reverse=True)
                        team_b_pl.append(team_b_bench.pop())
                        team_b_pl.sort(key=itemgetter(2),reverse=True)
                        team_b_pl.sort(key=itemgetter(1),reverse=True)
